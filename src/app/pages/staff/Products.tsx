...
export default function Products() {
  ...
  const franchiseLocations = [
    { label: "Main Store - Downtown", value: "loc-1" },
    { label: "Branch 2 - Westside Mall", value: "loc-2" },
    { label: "Branch 3 - Airport Plaza", value: "loc-3" },
  ];
  ...
  const handleEditProduct = () => {
    setProducts(products.map(p =>
      p.id === selectedProduct.id ? { ...selectedProduct, ...formData, locations: selectedLocations } : p
    ));
  ...
  const openEditDialog = (product: any) => {
    ...
    setSelectedLocations(product.locations || []);
      ...
      <Dialog open={isAddDialogOpen} onOpenChange={setIsAddDialogOpen}>
        <DialogContent className="max-w-2xl">
          ...
          <div className="grid md:grid-cols-2 gap-4 py-4">
            ...
            <div className="md:col-span-2 space-y-2">
              <Label>Apply to Franchise Locations (Optional)</Label>
              <MultiSelect
                options={franchiseLocations}
                selected={selectedLocations}
                onChange={setSelectedLocations}
                placeholder="Select locations..."
              />
              <p className="text-xs text-gray-500">
                Select which locations should have this product. Leave empty to add to current location only.
              </p>
            </div>
          </div>
          <DialogFooter>
          ...
          {selectedProduct && (
            <div className="space-y-4 py-4">
              <div className="grid md:grid-cols-2 gap-4">
                ...
                <div className="md:col-span-2">
                  <p className="text-sm text-gray-600 mb-2">Available at Locations</p>
                  <div className="flex flex-wrap gap-2">
                    {selectedProduct.locations && selectedProduct.locations.length > 0 ? (
                      selectedProduct.locations.map((locId: string) => {
                        const location = franchiseLocations.find(loc => loc.value === locId);
                        return (
                          <span
                            key={locId}
                      ...
                      <span className="text-gray-500 text-sm">Current location only</span>
                    )}
                  </div>
                </div>
              </div>
